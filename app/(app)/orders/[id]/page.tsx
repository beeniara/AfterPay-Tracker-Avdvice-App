...
export default async function OrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      ...
      <section className="flex items-center gap-4 rounded-card bg-primary px-5 py-5 rail:px-7 rail:py-6">
      ...
      <Card>
        <div className="grid gap-10 rail:grid-cols-2 rail:gap-12">
          <div>
            ...
            <dl>
              ...
              {ledger.totalFees > 0 ? <KeyValue label="Fees added" value={`+${money(ledger.totalFees)}`} /> : null}
              {ledger.amountRefunded > 0 ? <KeyValue label="Refunded" value={`−${money(ledger.amountRefunded)}`} /> : null}
              <KeyValue label="Total owing" value={money(ledger.owedAmount)} />
              {ledger.pendingAmount > 0 ? <KeyValue label="Pending" value={money(ledger.pendingAmount)} /> : null}
              <KeyValue
                label="True cost"
          ...
          <div className="order-first rail:order-none">
            <Progress value={ledger.totalPaid} max={ledger.totalPaid + ledger.owedAmount} label="Share of this order paid" />
...
function KeyValue({ label, value, sub, emphasis }: { label: string; value: string; sub?: string; emphasis?: boolean }) {
  return (
