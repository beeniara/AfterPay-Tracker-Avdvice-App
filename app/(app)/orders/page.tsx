...
import { orderListQuerySchema, parseSearchParams, type OrderListQuery } from "@/lib/api/params";
...
const DEFAULT_QUERY: OrderListQuery = { offset: 0, limit: 25, orderBy: "purchasedAt", ascending: false };
...
export default async function OrdersPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return (
    <>
      ...
      <HeroCard heading="Total owing" className="flex flex-wrap items-center justify-between gap-4">
        <HeroFigure label={`${summary.activeOrders} active of ${summary.totalOrders} orders`} value={formatCents(summary.total, user.currency)} />
      ...
      <Card>
        ...
        {page.results.length === 0 ? (
          <EmptyState
            ...
            description={hasFilters ? "Try a different search or clear the filters." : "Add an order or import a CSV to get started."}
            action={
              hasFilters ? (
                <ButtonLink href="/orders" variant="ghost">Clear filters</ButtonLink>
              ) : (
                <ButtonLink href="/orders/new">Add an order</ButtonLink>
              )
            }
          />
        ) : (
          <>
            <p className="mb-3 text-caption text-ink-muted">{page.totalResults.toLocaleString("en-NZ")} orders</p>
            <OrdersTable orders={page.results} />
            <Pagination totalResults={page.totalResults} offset={query.offset} limit={query.limit} hrefFor={hrefFor} />
          </>
        )}
      </Card>
    </>
